...
//    上传图片
    public function upload_cover()
    {
        ...
        $url = (new Upload())->upload_img($img,'info');
        if(!$url){
...
//    添加活动
    ...
    public function set_cond($id)
    {
        ...
        $res = (new InfoModel())->set_is($data,$id);
        if($res){
...
//修改活动
    public function do_update()
    {
        ...
        $res = (new InfoModel())->updateInfo($request,$id);
        if($res){
...
//        获取筛选条件
        ...
        $data = (new InfoModel())->search($search,10);
        $this->assign('data',$data);
